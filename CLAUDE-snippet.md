...
条文只写"做什么"，怎么做在四个 skill 里——**skill 正文只在被调用时才进上下文，条文本身常驻**，所以条文越短越好，别把 skill 内容抄进来。
...
- 执行会话开工先 set_session_title 自报板上名；交货 = 双层交货件（机读 + 给人看）+ 回填案 E 行；
  执行体只写自己那行 E，不读案全文。
- 会话过 150–200k 或到批界：主动提议收口换生（ctx-handoff）；继任开题只读案文件、禁读旧会话转录（ctx-takeover）。
...
3. 把会话养到 150k 以上，看它是否**主动提收口**（不提就是条文没吃进去，检查 CLAUDE.md 是否被加载）。

## 按项目裁剪

- 案目录名不同：把条文里的 `_ops/CASES/` 换成你的实际目录（四个 skill 会自动优先用已存在的目录，条文改不改都能跑）。
