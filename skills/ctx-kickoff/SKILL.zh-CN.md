...
开工第一动作：set_session_title 设为 "<父号>.<子序号>-<案号紧凑形>-<这一任在做啥>-执行"（如 `07.1-C07-压测跑批-执行`；散活无案号、中间段空着；子序号在派发者内自增，读会话清单里已有的 `<父号>.x` 取最大 +1；板上已派名则用派名）。**开工第二动作：把这个名字填回板上那件活的「载体」栏（散活行）或案 E 行的载体列**——只填自己那一格。漏填=盘上认不出你归谁，你会以"孤儿"身份出现在登记簿里，周检的退役补扫可能把正在干活的你标成死的（实测险情一次，只因当时是热的才躲过）。
