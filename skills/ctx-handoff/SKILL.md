...
- **Splitting F**: once section F alone passes 10,000 characters, cut it at a milestone boundary — everything before that milestone moves **verbatim** into `<case file name without .md>_chronicle-archive_<date>.md`, and the case keeps the entries since that milestone plus one pointer line. F is not part of the takeover load, but an unbounded chronicle makes every close-out that re-reads it dearer. **A decision a later decision has overturned moves into the decision appendix** as well (the old row goes across verbatim; the row in C that overturned it stays) — the same move as handing the old rows of C up, one row at a time instead of a block.
